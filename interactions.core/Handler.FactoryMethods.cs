...
public static class Handler {
  ...
  [Pure]
  public static AsyncHandler<Unit, T> FromAsyncMethod<T>(AsyncFunc<T> action) {
    ExceptionsHelper.ThrowIfNull(action, nameof(action));
    return FromAsyncMethod<Unit, T>((_, token) => action(token));
  }
  ...
  [Pure]
  public static AsyncHandler<T, Unit> FromAsyncMethod<T>(AsyncAction<T> action) {
    ExceptionsHelper.ThrowIfNull(action, nameof(action));
    return FromAsyncMethod<T, Unit>(async (input, token) => {
      await action(input, token);
      return default;
    });
  }
  ...
  /// <param name="action">Async action used for handling.</param>
  [Pure]
  public static AsyncHandler<Unit, Unit> FromAsyncMethod(AsyncAction action) {
    ExceptionsHelper.ThrowIfNull(action, nameof(action));
    return FromAsyncMethod<Unit, Unit>(async (_, token) => {
      await action(token);
      return default;
    });
  }
