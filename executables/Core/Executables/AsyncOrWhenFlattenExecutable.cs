...
internal sealed class AsyncOrWhenFlattenExecutable<T1, T2>(
  Func<T1, bool> condition,
  IAsyncExecutable<T1, Optional<T2>> inner,
  ...
  private readonly IAsyncExecutor<T1, Optional<T2>> _other = other.GetExecutor();
  ...
  async ValueTask<Optional<T2>> IAsyncExecutor<T1, Optional<T2>>.Execute(T1 input, CancellationToken token) {
    ...
    return result.HasValue ? result : condition(input) ? await _other.Execute(input, token) : Optional<T2>.None;
  }
