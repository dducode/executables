...
internal sealed class OrWhenExecutable<T1, T2>(Func<T1, bool> condition, IExecutable<T1, Optional<T2>> inner, IExecutable<T1, T2> other)
  ...
  Optional<T2> IExecutor<T1, Optional<T2>>.Execute(T1 input) {
    ...
    return result.HasValue ? result : condition(input) ? new Optional<T2>(_other.Execute(input)) : Optional<T2>.None;
  }
