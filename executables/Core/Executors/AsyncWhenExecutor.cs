...
internal sealed class AsyncWhenExecutor<T1, T2>(Func<T1, bool> condition, IAsyncExecutor<T1, T2> executor) : IAsyncExecutor<T1, Optional<T2>> {
  ...
  private static async ValueTask<Optional<T2>> Await(T1 input, IAsyncExecutor<T1, T2> executor, CancellationToken token) {
