...
public static partial class LinqExecutableExtensions {
  ...
  [Pure, EditorBrowsable(EditorBrowsableState.Never)]
  public static IExecutable<T1, T4> SelectMany<T1, T2, T3, T4>(
    this IExecutable<T1, T2> executable,
    Func<T2, IExecutable<T2, T3>> selector,
    Func<T2, T3, T4> resultSelector) {
    return executable.Bind(Executable.Create((T2 x) => selector(x).Then(y => resultSelector(x, y))));
  }

  [Pure, EditorBrowsable(EditorBrowsableState.Never)]
  public static IExecutable<T1, T2> Where<T1, T2>(this IExecutable<T1, T2> executable, Func<T2, bool> predicate) {
