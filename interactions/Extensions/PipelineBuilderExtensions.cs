...
public static partial class PipelineBuilderExtensions {
  ...
  /// <param name="func">Terminal function used as final handler.</param>
  /// <returns>Composed handler ready to be attached to handleable.</returns>
  [Pure]
  public static Handler<T1, T4> End<T1, T2, T3, T4>(this PipelineBuilder<T1, T2, T3, T4> builder, Func<T2, T3> func) {
    return builder.End(Handler.FromMethod(func));
  }

  /// <summary>
  /// Finalizes pipeline by converting parameterless function into terminal handler.
  /// </summary>
  /// <typeparam name="T1">Input type of the composed handler.</typeparam>
  /// <typeparam name="T2">Output type returned by terminal parameterless function.</typeparam>
  /// <typeparam name="T3">Output type of the composed handler.</typeparam>
  /// <param name="builder">Builder being finalized.</param>
  /// <param name="func">Terminal parameterless function used as final handler.</param>
  /// <returns>Composed handler ready to be attached to handleable.</returns>
  [Pure]
  public static Handler<T1, T3> End<T1, T2, T3>(this PipelineBuilder<T1, Unit, T2, T3> builder, Func<T2> func) {
    return builder.End(Handler.FromMethod(func));
  }
