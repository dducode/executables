...
public static partial class HandlersExtensions {
  ...
  [Pure]
  public static AsyncAutoDisposeHandlerProvider<T1, T2> DisposeOnException<T1, T2>(this AsyncHandler<T1, T2> handler) {
    handler.ThrowIfNullReference();
    return new AsyncAutoDisposeHandlerProvider<T1, T2>(handler);
  }
